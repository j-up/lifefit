...
export async function POST(request: Request) {
  try {
    ...
    const { rawPressRelease, category } = body;
    ...
    const promptText = `
다음 정부 보도자료 원문을 꼼꼼히 분석하여, 일반 서민/청년/직장인들이 한눈에 혜택을 이해하고 활용할 수 있도록 미려하게 구성된 2026 정부 지원 혜택 및 세금 절세 가이드 블로그 포스트를 한국어로 생성해 줘.
...
---
[요구 사양]:
1. content는 HTML 형식이어야 하며, 제목 태그(<h2>, <h3>)와 문단 태그(<p>), 강조(<strong>), 목록(<ul>, <li>), 인용구(<blockquote>) 등을 조화롭게 활용해 가독성을 극한으로 올려줘. 본문 내에 불필요한 마크다운 기호(\`\`\`html 등)는 배제하고 순수 HTML 문자열만 제공할 것.
2. slug는 이 포스트의 성격을 영어 단어 2~4개의 조합으로 나타낸 URL 친화적 문자열(예: 'youth-rent-benefit-2026')로 생성해 줘.
3. image는 이 글에 가장 어울리는 고품질 Unsplash 대표 이미지 URL을 선택해 줘. 아래 테마 중 어울리는 주소를 그대로 제공해야 해:
